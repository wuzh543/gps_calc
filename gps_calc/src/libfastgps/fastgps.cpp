...
int run_fastgps()
{
  ...
  control_file = fopen("control_file.txt", "r");//^D指示的文件
  bit_file = fopen("bit_file.txt", "r");
  calc_file = fopen("calc_file.txt", "r");
  ...
  if (!control_file)
  {
    fastgps_printf("ahhhh couldn't open control file.\nCheck that path to data "
  ...
  if (!bit_file)
  {
	  fastgps_printf("ahhhh couldn't open bit file.\nCheck that path to data "
		  "file in fastgps_config.txt is correct.\n");
	  return -1;
  }
  ...
  if (!calc_file)
  {
	  fastgps_printf("ahhhh couldn't open calc file.\nCheck that path to data "
		  "file in fastgps_config.txt is correct.\n");
	  return -1;
  }
...
*/
  ...
  //Read fundmental information
  system_vars.loop_count = 1;//init
  fscanf(control_file, "%d ", &system_vars.num_channels);
  for (unsigned chan = 0; chan < MAX_CHANNELS; chan++)
  {
	  fscanf(control_file, "%d ", &c[chan].prn_num);
  }
 ...
 while (!feof(control_file) && (system_vars.process_time < system_vars.run_time))
  {
    //// Read Data from file
    ...
    unsigned int ch_idx;  //count max channel
	...
	for (unsigned chan = 0; chan < MAX_CHANNELS; chan++)
	{    
		fscanf(control_file, "%d ",&c[chan].chan_bit_flag);
		//fscanf(data_file, "%d %d %d ",
		//	&c[chan].prn_num, &c[chan].ip, &c[chan].qp);
		//fscanf(control_file, "%lf ", &c[chan].doppler);
		//if (c[chan].ip != 0)
		//{
	...
	fscanf(control_file, "%d ", &system_vars.calc_flag);
	//fprintf(system_vars.tracking_log, "\n");

	if (system_vars.calc_flag == 1)
	{
		fscanf(calc_file, "%d ", &system_vars.loop_count);
		for (unsigned chan = 0; chan < MAX_CHANNELS; chan++)
		{
			fscanf(calc_file, "%d %d %lf %lf ", &c[chan].state_ms,
    ...
    //{
	  ...
	  int navbit;
	  navbit = 0;
	  //fscanf(bit_file, "%d %d ",&navbit, &prnnum);
      ...
      for (ch_idx = 0; ch_idx < system_vars.num_channels; ch_idx++)
      {
		...
		if (prnnum == c[ch_idx].prn_num)
		{
			process_nav_bit(&c[ch_idx], navbit);
		}
		*/

		if (c[ch_idx].chan_bit_flag == 1)
		{
			fscanf(bit_file, "%d ", &navbit);
			process_nav_bit(&c[ch_idx], navbit);
		}
      ...
      /* *********************************** */
	  if ((system_vars.num_valid_eph >= 1) && (system_vars.calc_flag == 1))                     //刚开始的主要限制
